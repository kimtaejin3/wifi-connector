...
■ 한 번 연결한 곳은 기록에
연결한 Wi-Fi는 기록 탭에 남아 다음에 촬영 없이 다시 연결할 수 있습니다. 기록은 기기의 키체인에만 암호화되어 저장됩니다.
...
6.9″ 디스플레이용(1320×2868) 4장이 `store/`에 있습니다. App Store Connect의 "iPhone 6.9″ 디스플레이"에 순서대로 올리면 작은 화면용은 자동으로 줄여 씁니다.

| 순서 | 파일 | 상단 문구 | 화면 |
|---|---|---|---|
| 1 | `store/s1.png` | 와이파이 안내문을 비추면 바로 읽어요 | 카메라로 안내문을 비추고 "Wi-Fi 정보를 찾았어요" 배너 |
| 2 | `store/s2.png` | 이름과 비밀번호를 정확하게 | 결과 화면과 Wi-Fi 연결 버튼 |
| 3 | `store/s3.png` | 승인 한 번이면 연결 끝 | "Wi-Fi에 연결되었습니다" |
| 4 | `store/s4.png` | 다녀간 곳은 기록에서 다시 | 기록 탭 |
